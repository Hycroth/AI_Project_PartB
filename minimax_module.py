...
class Player:
    ...
    def action(self, turns):
        ...
        if self.phase == PLACING:
            # Get position to place piece
            next_action = self.alpha_beta_place(PLACE_DEPTH)
            
            # Place the piece on our game board
    ...
    # Finds best placing move using minimax with alpha beta pruning
    def alpha_beta_place(self, depth):
        values = {} # dictionary of placing moves and their minimax value
        
        # Iterate through all available squares we could place a piece
        for square in self.board.starting_zone(self.colour):
            if self.board.get_piece(square) == None:
                eliminated = self.board.place_piece(self.colour, square)
                values[square] = self.min_place(depth-1, math.inf, -math.inf)
                self.board.undo_place(self.colour, square, eliminated)
                
        return max(values, key=values.get)
    
    # MIN will always be our opponents turn in this case
    def min_place(self, depth, a, b):
        
        # Cutoff test: either end of placing phase or depth limit
        if (self.turns + (PLACE_DEPTH - depth)) == MOVING_PHASE or depth == 0:
            return self.evaluate_board(self.board)
        
        for square in self.board.starting_zone(self.enemy):
            if self.board.get_piece(square) == None:
                eliminated = self.board.place_piece(self.enemy, square)
    ...
    # MAX will always be our players turn in this case
